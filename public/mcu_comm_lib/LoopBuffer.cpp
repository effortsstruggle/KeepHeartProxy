...
#define  LLOG(...) __android_log_print(ANDROID_LOG_DEBUG, "SKY_MCU", __VA_ARGS__);
...
int LoopBuffer::GetData(unsigned char* pBuf, int nLength)
{
	...
	while(nRecved<nLength && nTimes<=1)
	{
		...
		if(nSubRecved > 0)
		{
			...
			m_nLen -= nSubRecved;
		//	LLOG("---- nSubRecved:%d,m_nLen :%d",nSubRecved,m_nLen);
			nRecved += nSubRecved;
		}
...
{
	//LLOG("LoopBuffer::AddData :%d",nDataLength);
	...
	while(nDataLength > 0)
	{
		...
		m_nLen += nSaved;
		// LLOG("+++ nSaved:%d,m_nLen :%d",nSaved,m_nLen);
		pData += nSaved;
		nDataLength -= nSaved;
	}
...
void LoopBuffer::Pop(int count)
{
    ...
    m_nLen -= count; 
//	LLOG("---- count:%d,m_nLen :%d",count,m_nLen);
}
